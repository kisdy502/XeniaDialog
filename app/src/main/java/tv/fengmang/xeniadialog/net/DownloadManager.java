...
public class DownloadManager {
    ...
    private static HashMap<String, Call> mCallMap = new HashMap<>();
    ...
    public void cancel(String downloadUrl) {
        ...
        if (Build.VERSION.SDK_INT >= 24) {
            mCallMap.remove(downloadUrl, call);
        } else {
    ...
    public boolean download(String downloadUrl, String saveDir, String saveFileName, @Nullable
        ...
        try {
            ...
            if (response.isSuccessful()) {
                ...
                return saveFile(response, file, start, downloadUrl, listener);
            } else {
    ...
    private boolean saveFile(Response response, File saveFile, long start, String downloadUrl, ProgressListener listener) {
        InputStream is = null;
        ...
        try {
            ...
            listener.onFailed(downloadUrl, ProgressListener.FAILED_SAVE_FILE, e.getMessage());
            return false;
        } finally {
            close(bis);
            close(is);
    ...
    private long getContentLength(String downloadUrl) {
        long contentLength = TOTAL_ERROR;
        Request request = new Request.Builder().url(downloadUrl).build();
        try {
            Response response = new OkHttpClient.Builder().build().newCall(request).execute();
            int code = response.code();
            Log.i("dm", "response:" + code);
            if (code == 200) {
                contentLength = response.body().contentLength();
                Log.i("dm", "response.contentLength:" + contentLength);
                //response.close(); 3.2后 RequestBody未实现Closeable接口，没有这个方法
            } else {
                contentLength = RESPONSE_CODE_ERROR;
            }
        } catch (IOException e) {
            e.printStackTrace();
            contentLength = REQUEST_EXCEPTION;
        }
